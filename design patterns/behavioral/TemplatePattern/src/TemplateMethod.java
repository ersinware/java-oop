...
class Data {
    ...
    static public int getCreditScore() {
        return 650;
    }
}
...
abstract class CheckBackground {
    ...
    public void check() {
        prepareApplication();
        Boolean status = checkBank() && checkCredit() && checkLoan() && checkStock() && checkIncome();
...
class MortgageLoanApp extends CheckBackground {
    public MortgageLoanApp(String name) {
        _name = name;
    }
...
class EquityLoanApp extends CheckBackground {
    public EquityLoanApp(String name) {
        _name = name;
    }
